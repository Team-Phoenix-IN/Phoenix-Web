...
(function () {
  ...
  function clearHistory() {
    ...
    // Cloud sync
    if (currentUser) {
      setDoc(doc(db, 'users', currentUser.uid), { trackerHistory: [] }, { merge: true }).catch(() => { });
    }
  }
  ...
  function initAuth() {
    ...
    if (googleLoginBtn) {
      googleLoginBtn.addEventListener('click', () => {
        signInWithPopup(auth, googleProvider).then(() => closeLoginModal()).catch(err => showError(err.message));
      });
    }
    if (emailLoginBtn) {
      emailLoginBtn.addEventListener('click', () => {
        signInWithEmailAndPassword(auth, emailInput.value, passwordInput.value)
          .then(() => closeLoginModal()).catch(err => showError(err.message));
      });
    }
    if (emailSignupBtn) {
      ...
      settingsSaveBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (selectedImageFile) {
            if (settingsMsg) { settingsMsg.style.color = '#fff'; settingsMsg.textContent = 'Uploading image...'; }
          ...
          const newName = settingsDisplayName ? settingsDisplayName.value.trim() : '';
          await updateProfile(currentUser, { displayName: newName || currentUser.displayName, photoURL: newPhotoUrl });
          const riotId = settingsRiotId ? settingsRiotId.value.trim() : '';
          await setDoc(doc(db, 'users', currentUser.uid), { riotId }, { merge: true });
    ...
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          ...
          if (snap.exists() && snap.data().riotId && settingsRiotId) settingsRiotId.value = snap.data().riotId;
        } catch (e) { }
        // Sync history from cloud
        try {
          const snap = await getDoc(doc(db, 'users', user.uid));
          if (snap.exists() && snap.data().trackerHistory) {
            localStorage.setItem(HISTORY_KEY, JSON.stringify(snap.data().trackerHistory));
            renderHistory();
          }
        } catch (e) { }
      } else {
