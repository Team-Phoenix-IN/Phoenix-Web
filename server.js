...
// ─── CUSTOM DISCORD BOT FOR PRESENCE ───
...
async function initDiscordBot() {
    ...
    if (!token) {
        console.log('⚠️ DISCORD_BOT_TOKEN not found in .env. Skipping Discord bot initialization.');
        return;
    }
...
// Endpoint to fetch presence for specific users
app.get('/api/discord/presence', (req, res) => {
    ...
    userIds.forEach(id => {
        // Find the user in any guild the bot is in
        ...
        if (userPresence) {
            // Map discord.js presence to the format our frontend expects (similar to Lanyard)
            const activities = userPresence.activities.map(a => ({
                name: a.name,
                type: a.type,
                state: a.state,
                details: a.details
            ...
            presenceData[id] = {
                discord_status: userPresence.status, // 'online', 'idle', 'dnd', 'offline'
                activities: activities
            };
        } else {
            // Default to offline if user not found or has no active presence
            presenceData[id] = {
